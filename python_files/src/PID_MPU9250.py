...
Kp = 1.9 #1.5 #0.6    #Proportional PID constant
Ki = 0.6 #0.08 #.06    #Integral PID constant
Kd = 0.48 #0.5 #.2    #Derivative PID constant
...
print("WAMI:",read_byte(MPU_ADD,0x75))
prevtime = time.time()
initial = prevtime
...
#print("CF_x_init: {} CF_y_init: {}".format(CF_x, CF_y))
...
try:
    while True:    #keep running until a key press
        ...
        #set reference step
#        if (prevtime - initial > 5 and prevtime - initial < 5.5):
#            ref[0] = -4.5
        #read accelerometer and gyroscope raw values and get the scaled values
        (gyro_scaled_x, gyro_scaled_y, gyro_scaled_z, accel_scaled_x, accel_scaled_y, accel_scaled_z) = read_all()
...
#            pitch_pid=-(100-thrust)
        ...
        #print ("elapsed:{0:.4f} Acc_x:{1:.2f} Gyr_T_x:{2:.2f} CF_x:{3:.2f} Acc_y:{4:.2f} Gyr_T_y:{5:.2f} CF_y:{6:.2f}".format(time.time()-initial,(rotation_x),(gyro_total_x),(CF_x),(rotation_y),(gyro_total_y),(CF_y)))
